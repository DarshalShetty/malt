...
(require "1-flats.ss")
(require "2-acc-runtime.rkt")
(require "2-acc-kernels.rkt")
(require "2-acc-fun-to-prealloc.rkt")
(require "B-tensor-basics.ss")
(require "C-tensor-ops.ss")
(require "D-fun-to-prealloc.ss")
...
(define ext1-ρ
  (λ (f f-acc m
        ...
        [prim-sig (ext1-ρ-gensig)])
    (λ (t)
      (cond
        ((number? t) (f t))
        ((expects-preallocated? f-acc)
         (scalarize
          (flat-ext1-ρ f f-acc m shape-fn prim-sig t)))
        (else
         (let* ((in-shape (flat-shape t))
           ...
           (scalarize
            (flat-ext1-ρ flat-f flat-f-acc m shape-fn prim-sig t))))))))

(define ext1-∇
  (let ((id -1))
    (λ (f f-acc m
        [shape-fn scalar-shape]
        [prim-sig (ext1-∇-gensig)])
      (λ (t z)
        (cond
          ((number? t) (f t z))
          ((expects-preallocated? f-acc)
           (scalarize (flat-ext1-∇ f f-acc m shape-fn prim-sig t (ensure-flat z))))
          (else
           (let* ((in-shape (flat-shape t))
                  (base-shape (min-shape m in-shape))
                  (out-shape (shape-fn base-shape))
...
(define ext2-ρ
  (λ (f f-acc m n
        [shape-fn scalar-shape]
        [prim-sig (ext2-ρ-gensig)])
    (λ (t u)
      (cond
        ((and (number? t) (number? u)) (f t u))
        ((expects-preallocated? f-acc)
         (scalarize
          (flat-ext2-ρ f f-acc m n shape-fn prim-sig t u)))
        ((number? t)
         (let* ((t-shape '())
           ...
           (scalarize
            (flat-ext2-ρ flat-f flat-f-acc m n shape-fn prim-sig (ensure-flat t) u))))
        ((number? u)
         (let* ((t-shape (min-shape m (flat-shape t)))
                ...
                (flat-f-acc (functional->preallocated-2-ρ-acc f-acc t-shape u-shape out-shape)))
           (scalarize
            (flat-ext2-ρ flat-f flat-f-acc m n shape-fn prim-sig t (ensure-flat u)))))
        (else
         (let* ((t-shape (min-shape m (flat-shape t)))
                (u-shape (min-shape n (flat-shape u)))
                (out-shape (shape-fn t-shape u-shape))
                ...
                (flat-f-acc (functional->preallocated-2-ρ-acc f-acc t-shape u-shape out-shape)))
           (scalarize
            (flat-ext2-ρ flat-f flat-f-acc m n shape-fn prim-sig t u))))))))

(define ext2-∇
  (λ (f f-acc m n
        [shape-fn scalar-shape]
        [prim-sig (ext2-∇-gensig)])
    (λ (t u z)
      (let ((invoke-flat-ext2-∇
             (λ (f f-acc m n shape-fn t u z)
               (let-values (((da db) (flat-ext2-∇ f f-acc m n shape-fn prim-sig t u z)))
...
(define flat-ext1-ρ
  (λ (f f-acc min-rank shape-fn f-sig t0)
    (let* ((s0 (flat-shape t0))
      ...
      (cond
        ((accelerate?)
         (let-values (((kernel-code kernel-name) (ext1-ρ-kernel/name f-acc f-sig)))
...
(define flat-ext1-∇
  (λ (fᵈ fᵈ-acc min-rank shape-fn fᵈ-sig t0 z)
    ...
    (let* ((s0 (flat-shape t0))
      ...
      (cond
        ((accelerate?)
         (let-values (((kernel-code kernel-name) (ext1-∇-kernel/name fᵈ-acc fᵈ-sig)))
...
(define flat-ext2-ρ
  (λ (f f-acc r0 r1 shape-fn f-sig t0 t1)
    (let* ((s0 (flat-shape t0))
           (v0 (flat-store t0))
           (off0 (flat-offset t0))
           (sf0 (min-shape r0 s0))
      ...
      (ext2-shapes s0 s1 r0 r1 sf-out
        (λ (s-out size-out q0 q1 strides)
          (let ((out-v (new-vec size-out 0.0)))
            (cond
              ((accelerate?)
               (let-values (((kernel-code kernel-name) (ext2-ρ-kernel/name f-acc f-sig strides)))
...
(define flat-ext2-∇
  (λ (fᵈ fᵈ-acc r0 r1 shape-fn fᵈ-sig t0 t1 z)
    (let* ((s0 (flat-shape t0))
           (v0 (flat-store t0))
           (off0 (flat-offset t0))
           (sf0 (min-shape r0 s0))
            ...
            (cond
              ((accelerate?)
               (let-values (((kernel-code kernel-name)
                             (ext2-∇-kernel/name fᵈ-acc fᵈ-sig strides s0 s1 r0 r1 sz
                                                 (length sf-z))))
...
(define ext2-shapes
  (let ((cache (make-hash)))
    (λ (s0 s1 r0 r1 sf-out k)
      (let ((cache-key (equal-hash-code (list s0 s1 r0 r1 sf-out))))
        (cond
